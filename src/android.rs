...
pub trait Android {
    fn swipe(&self, x0: usize, y0: usize, x1: usize, y1: usize, duration: usize) {
        Command::new("adb")
            .args(&[
                "shell",
                "input",
    ...
    fn xpath(&self, xpath_rule: &str) -> Vec<String> {
    ...
    fn texts(&self, rule: &str) -> Vec<String> {
        self.uiautomator();
        dbg!(rule);
        let xpath_rule = &CONFIG[rule];
        return self.xpath(xpath_rule);
    }
    fn positions(&self, rule: &str) -> Vec<(usize, usize)> {
